...
if (isset($_POST["number"])) {
    $gussed = true;
    $secretNumber = $_POST["number"];
    $tries = $_POST["tries"];
    $yourGuess = $_POST["guess"];
    ...
    <body>
        ...
        <div class="d-flex justify-content-center">
            <div class="form-group w-50">
                <h1 class="text-center">Guess the number! (Post)</h1>
                <?php if ($gussed) : ?>
                    <?php if ($guess->makeGuess($yourGuess)) : ?>
                        <h2>You guessed right with the number <?= $yourGuess ?> with <?= $guess->tries() ?> guesses remaining.</h1>
                    <?php else : ?>
                        <h2>You have <?= $guess->tries() ?> guesses left.</h2>
